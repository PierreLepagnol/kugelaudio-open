...
class SConv1d(nn.Module):
    ...
    def _forward_non_streaming(self, x: torch.Tensor, debug: bool = False) -> torch.Tensor:
        ...
        if (
            hasattr(self, "conv")
            and hasattr(self.conv, "conv")
            and hasattr(self.conv.conv, "weight")
        ):
...
class SConvTranspose1d(nn.Module):
    ...
    def _forward_non_streaming(self, x: torch.Tensor, debug: bool = False) -> torch.Tensor:
        ...
        if (
            hasattr(self, "convtr")
            and hasattr(self.convtr, "convtr")
            and hasattr(self.convtr.convtr, "weight")
        ):
...
class TokenizerEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Use pure Python to avoid meta tensor issues with device_map="auto"
        n = sum(self.depths)
...
class KugelAudioAcousticTokenizerModel(PreTrainedModel):
    ...
    def decode(self, latents, cache=None, sample_indices=None, use_cache=False, debug=False):
        ...
        audio = self.decoder(
            latents, cache=cache, sample_indices=sample_indices, use_cache=use_cache, debug=debug
        )
        return audio
